...
BlurShaderHor::BlurShaderHor(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"horizontalBlur_vs.cso", L"horizontalBlur_ps.cso");
}


BlurShaderHor::~BlurShaderHor()
{
...
void BlurShaderHor::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void BlurShaderHor::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float screen_width)
{
	HRESULT result;
	...
	screenPtr->screen_width = screen_width;
...
void BlurShaderHor::render(ID3D11DeviceContext* deviceContext, int indexCount)
{
