...
TessellationBlendingShader::TessellationBlendingShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"tessellation_vs.cso", L"tessellation_hs.cso", L"tessellation_ds.cso", L"tessellationBlending_ps.cso");
}
...
void TessellationBlendingShader::initShader(WCHAR* vsFilename,  WCHAR* psFilename)
{
...
void TessellationBlendingShader::initShader(WCHAR* vsFilename, WCHAR* hsFilename, WCHAR* dsFilename, WCHAR* psFilename)
{
...
void TessellationBlendingShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float displacementMod, ID3D11ShaderResourceView* heightMap, Camera* camera, bool usesSpecMap, ID3D11ShaderResourceView* specMap, std::vector<MyLight*> lights, ID3D11ShaderResourceView* shadowMap, XMFLOAT4 tessellationFactors, ID3D11ShaderResourceView* alphaMap, ID3D11ShaderResourceView* blendTexture)
{
	...
	deviceContext->PSSetShaderResources(3, 1, &alphaMap);
	deviceContext->PSSetShaderResources(4, 1, &blendTexture);
	deviceContext->DSSetShaderResources(0, 1, &heightMap);
}

void TessellationBlendingShader::setFogParameters(ID3D11DeviceContext* deviceContext, bool activeFog, float fogStart, float fogRange, float maxFogAmnt, XMFLOAT3 fogColour)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
...
void TessellationBlendingShader::render(ID3D11DeviceContext* deviceContext, int indexCount)
{
