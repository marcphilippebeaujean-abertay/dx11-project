...
BlurShaderVer::BlurShaderVer(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"verticalBlur_vs.cso", L"verticalBlur_ps.cso");
}


BlurShaderVer::~BlurShaderVer()
{
...
void BlurShaderVer::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void BlurShaderVer::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float screen_height)
{
	HRESULT result;
	...
	screenPtr->screen_height = screen_height;
...
void BlurShaderVer::render(ID3D11DeviceContext* deviceContext, int indexCount)
{
